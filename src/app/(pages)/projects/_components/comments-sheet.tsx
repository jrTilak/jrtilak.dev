  ...
  return (
    <>
      ...
      <Sheet open={isOpen} onOpenChange={seIsOpen}>
        <SheetContent className="flex w-[350px] flex-col gap-4 p-0">
          <div className="scrollbar flex flex-grow flex-col gap-3 overflow-y-auto p-4 pb-0">
            <SheetHeader className="mb-4">
              <SheetTitle>Share Your Thoughts</SheetTitle>
              <SheetDescription className="text-xs">
                Leave a comment below for any questions, suggestions, or thoughts you have regarding
                this post.
              </SheetDescription>
            </SheetHeader>
            {isCommentsLoading ? (
              ...
              <p className="text-muted-foreground h-full min-h-12 w-full text-sm">
                No comments yet!
                <br />
                Start by writing your thoughts. 💬
              </p>
            ) : (
              comments?.map((comment, i) => <CommentComponent key={i} comment={comment} />)
            )}
          </div>
          <SheetFooter className="flex w-full !flex-col gap-2 p-4 pt-0">
            <Textarea
            ...
            />
            <Button isLoading={isCommentAdding} className="w-full" onClick={commentOnBlog}>
              Chime In!
